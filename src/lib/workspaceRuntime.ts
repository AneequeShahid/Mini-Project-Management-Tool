...
const runtime = globalThis as typeof globalThis & { pulseRuntime?: { events: LocalEvent[]; meetings: typeof MEETINGS; integrations: LocalIntegration[] } };

export const workspaceRuntime = runtime.pulseRuntime ??= {
...
export function scheduleLocalMeeting(input: { title: string; date: string; time: string; duration?: string; type?: string; attendees?: number }) {
  ...
  const joinUrl = `https://meet.jit.si/pulse-${room}-${id.slice(-6)}`;
  ...
  workspaceRuntime.events.unshift({ id, title: input.title, description: 'Scheduled with Pulse local meeting mode.', startTime, endTime, provider: 'local', link: joinUrl, meetLink: joinUrl, attendees: [] as string[] });
  return meeting;
}
